...
func marshalTo(source map[string]interface{}, outputStruct interface{}) error {
...
func handleFlattenedLists(source map[string]interface{}) error {
...
func remapDataValues(source map[string]interface{}) error { //nolint
	...
	for k, propertyValue := range source {
		if strings.HasSuffix(k, "CSV") { // Properties ending in "CSV" will, as a convenience, be split into a list on the fly
			...
			case map[string]interface{}:
				...
				delete(source, k)
			default:
				return fmt.Errorf("unexpected value type %+v - should be string or map[string]interface{}", propertyValueTyped)
			}
